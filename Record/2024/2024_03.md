...
private async Awaitable AwatableTest()
{
    //Request 사용 이유: XmlReader.Create()에서 string으로 path를 받으면 WebGL로 빌드했을때 못 읽어옴.
    //따라서 한번 StringReader로 변환후 가져와야 잘 됨.
    using (UnityWebRequest www = UnityWebRequest.Get(_fileUrl))
    {
        ...
        StringReader xmlText = new StringReader(testText);
        Debug.Log("k확인 xmlText: " + k);
